angular.module('myApp').controller('HomeController', function ($scope, $http, $q, $location, AuthService) {
  $scope.query = ''
  var userId
  $scope.search = function () {
    ...
    $http.get(`http://localhost:5000/products/search/${$scope.query}`)
  ...
  $scope.changeQuantity = function (item, change) {
    ...
    const productId = product._id
    $http.patch(`http://localhost:5000/users/${userId}/cart/${change}/${productId}`)
      .then(function (response) {
        item.quantity = response.data.newUser.cart[productId.toString()].quantity
      })
      .catch(function (error) {
        console.log(error)
      })
  ...
  $scope.purchaseItems = function () {
    ...
    $http.post(`http://localhost:5000/users/${userId}/purchase`)
      .then(function (response) {
        getProducts()
      })
      .catch(function (error) {
